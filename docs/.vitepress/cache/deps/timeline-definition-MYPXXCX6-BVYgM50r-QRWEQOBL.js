...
// node_modules/vitepress-mermaid-preview/dist/timeline-definition-MYPXXCX6-BVYgM50r.js
var Q = function() {
  var n = g(function(x, r, a, c) {
    ...
    performAction: g(function(r, a, c, u, y, o, w) {
      var v = o.length - 1;
      switch (y) {
        case 1:
          return o[v - 1];
    ...
    parse: g(function(r) {
      var a = this, c = [0], u = [], y = [null], o = [], w = this.table, v = "", N2 = 0, P = 0, W = 2, U = 1, H2 = o.slice.call(arguments, 1), g2 = Object.create(this.lexer), b = { yy: {} };
      for (var L in this.yy)
        Object.prototype.hasOwnProperty.call(this.yy, L) && (b.yy[L] = this.yy[L]);
      g2.setInput(r, b.yy), b.yy.lexer = g2, b.yy.parser = this, typeof g2.yylloc > "u" && (g2.yylloc = {});
      var $ = g2.yylloc;
      o.push($);
      var z = g2.options && g2.options.ranges;
      ...
      function tt() {
        var T;
        return T = u.pop() || g2.lex() || U, typeof T != "number" && (T instanceof Array && (u = T, T = u.pop()), T = a.symbols_[T] || T), T;
      }
      g(tt, "lex");
      for (var S, A, I, J, R = {}, B, M, et2, O; ; ) {
        if (A = c[c.length - 1], this.defaultActions[A] ? I = this.defaultActions[A] : ((S === null || typeof S > "u") && (S = tt()), I = w[A] && w[A][S]), typeof I > "u" || !I.length || !I[0]) {
          ...
          g2.showPosition ? K = "Parse error on line " + (N2 + 1) + `:
` + g2.showPosition() + `
Expecting ` + O.join(", ") + ", got '" + (this.terminals_[S] || S) + "'" : K = "Parse error on line " + (N2 + 1) + ": Unexpected " + (S == U ? "end of input" : "'" + (this.terminals_[S] || S) + "'"), this.parseError(K, {
            text: g2.match,
        ...
        switch (I[0]) {
          case 1:
            c.push(S), y.push(g2.yytext), o.push(g2.yylloc), c.push(I[1]), S = null, P = g2.yyleng, v = g2.yytext, N2 = g2.yylineno, $ = g2.yylloc;
            break;
          case 2:
...
` + this.showPosition(), {
      ...
      performAction: g(function(a, c, u, y) {
        switch (u) {
          case 0:
            break;
          case 1:
  ...
  return g(_, "Parser"), _.prototype = p, p.Parser = _, new _();
}();
Q.parser = Q;
var Tt = Q;
var at = {};
...
var G = [];
var V = [];
var ot = g(() => Ff, "getCommonDb");
var ct = g(function() {
  X.length = 0, G.length = 0, F = "", V.length = 0, Fb();
}, "clear");
var ht = g(function(n) {
  F = n, X.push(n);
}, "addSection");
...
var Lt = g(function(n, t) {
  ...
  function d(m) {
    const p = hn().startAngle(Math.PI / 2).endAngle(3 * (Math.PI / 2)).innerRadius(7.5).outerRadius(6.8181818181818175);
    m.append("path").attr("class", "mouth").attr("d", p).attr("transform", "translate(" + t.cx + "," + (t.cy + 2) + ")");
  }
  g(d, "smile");
  function h(m) {
    const p = hn().startAngle(3 * Math.PI / 2).endAngle(5 * (Math.PI / 2)).innerRadius(7.5).outerRadius(6.8181818181818175);
  ...
  return g(f, "ambivalent"), t.score > 3 ? d(i) : t.score < 3 ? h(i) : f(i), l;
}, "drawFace");
...
var mt = function() {
  ...
  return g(l, "_setTextAttrs"), function(i) {
...
function D(n, t) {
  n.each(function() {
    var e = dt(this), l = e.text().split(/(\s+|<br>)/).reverse(), i, d = [], h = 1.1, f = e.attr("y"), m = parseFloat(e.attr("dy")), p = e.text(null).append("tspan").attr("x", 0).attr("y", f).attr("dy", m + "em");
...
var zt = g(function(n, t, e, l) {
  var _a, _b, _c;
  const i = gt(), d = ((_a = i.timeline) == null ? void 0 : _a.leftMargin) ?? 50;
  N.debug("timeline", l.db);
  const h = i.securityLevel;
  let f;
  h === "sandbox" && (f = dt("#i" + t));
  const p = (h === "sandbox" ? dt(f.nodes()[0].contentDocument.body) : dt("body")).select("#" + t);
  p.append("g");
  const k = l.db.getTasks(), _ = l.db.getCommonDb().getDiagramTitle();
  N.debug("task", k), C.initGraphics(p);
  ...
  x.forEach(function(H2) {
    const g2 = {
      ...
      padding: 20,
      maxHeight: r
    }, b = C.getVirtualNodeHeight(p, g2, i);
    N.debug("sectionHeight before draw", b), r = Math.max(r, b + 20);
  });
  let N2 = 0, P = 0;
  N.debug("tasks.length", k.length);
  for (const [H2, g2] of k.entries()) {
    const b = {
      number: H2,
      descr: g2,
      section: g2.section,
    ...
    N.debug("taskHeight before draw", L), a = Math.max(a, L + 20), N2 = Math.max(N2, g2.events.length);
    let $ = 0;
    for (const z of g2.events) {
      const Z = {
        descr: z,
        section: g2.section,
        number: g2.section,
    ...
    g2.events.length > 0 && ($ += (g2.events.length - 1) * 10), P = Math.max(P, $);
  }
  N.debug("maxSectionHeight before draw", r), N.debug("maxTaskHeight before draw", a), x && x.length > 0 ? x.forEach((H2) => {
    const g2 = k.filter((z) => z.section === H2), b = {
      number: w,
      descr: H2,
      section: w,
      width: 200 * Math.max(g2.length, 1) - 50,
      padding: 20,
      maxHeight: r
    };
    N.debug("sectionNode", b);
    const L = p.append("g"), $ = C.drawNode(L, b, w, i);
    N.debug("sectionNode output", $), L.attr("transform", `translate(${y}, ${u})`), o += r + 50, g2.length > 0 && st(
      p,
      g2,
      w,
      y,
      ...
      r,
      false
    ), y += 200 * Math.max(g2.length, 1), o = u, w++;
  }) : (v = false, st(
    p,
    k,
    w,
  ...
  N.debug("bounds", W), _ && p.append("text").text(_).attr("x", W.width / 2 - d).attr("font-size", "4ex").attr("font-weight", "bold").attr("y", 20), c = v ? r + a + 150 : a + 100, p.append("g").attr("class", "lineWrapper").append("line").attr("x1", d).attr("y1", c).attr("x2", W.width + 3 * d).attr("y2", c).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)"), Ab(
    void 0,
    p,
    ((_b = i.timeline) == null ? void 0 : _b.padding) ?? 50,
    ((_c = i.timeline) == null ? void 0 : _c.useMaxWidth) ?? false
  );
}, "draw");
var st = g(function(n, t, e, l, i, d, h, f, m, p, k) {
  var _a;
  for (const _ of t) {
    const x = {
      descr: _.task,
    ...
    if (N.debug("taskHeight after draw", c), r.attr("transform", `translate(${l}, ${i})`), d = Math.max(d, c), _.events) {
...
var jt = g((n) => {
  ...
  for (let e = 0; e < n.THEME_COLOR_LIMIT; e++)
    n["lineColor" + e] = n["lineColor" + e] || n["cScaleInv" + e], Ln(n["lineColor" + e]) ? n["lineColor" + e] = H(n["lineColor" + e], 20) : n["lineColor" + e] = et(n["lineColor" + e], 20);
  for (let e = 0; e < n.THEME_COLOR_LIMIT; e++) {
    const l = "" + (17 - 3 * e);
    t += `
...
//# sourceMappingURL=timeline-definition-MYPXXCX6-BVYgM50r-QRWEQOBL.js.map
